...
# 加载系统提示词--读取系统提示词文件里的内容
def load_system_prompts():
    try:
        system_prompt_path = get_abs_path(prompts_conf["main_prompt_path"])  #获取系统提示词文件的绝对路径
    except KeyError as e:
        logger.error(f"[load_system_prompts]在yaml配置项中没有main_prompt_path配置项")
        raise e

    try:
        return open(system_prompt_path, "r", encoding="utf-8").read()  #读取系统提示词文件里的内容
